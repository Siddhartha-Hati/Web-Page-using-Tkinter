...
# Function to update user password if old password is correct
def update_password():
    try:
        username = username_entry.get()  # Get the username from the entry field
        old_password = old_password_entry.get()  # Get the old password from the entry field
        new_password = new_password_entry.get()  # Get the new password from the entry field
        confirm_password = confirm_password_entry.get()  # Get the confirmed password from the entry field
        ...
        if new_password == confirm_password:
            ...
            cursor.execute("SELECT * FROM admindetails WHERE Name = %s AND passwords = %s", (username, old_password))
            user_data = cursor.fetchone()

            if user_data:
                # If the username and old password match, update the user's password
                update_query = "UPDATE admindetails SET passwords = %s WHERE Name = %s"
...
app= tk.Tk()
app.title('powerpro.com/Admin Function Page/Change-Admin_password')
