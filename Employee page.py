...
def check_login():
    ...
    try:
        ...
        select_query = "SELECT * FROM employeedetails WHERE Name = %s AND passwords = %s"
...
def Newregistration():
    root.destroy()
    call(['python','employee login details.py'])
def back():
    root.destroy()
...
root= tk.Tk()
root.title('powerpro.com/Employee page')
...
adminlogo=tk.Label(root,text='EMPLOYEE',bg='Black',fg='White',font='Elephant',padx=15).place(x=210,y=135)
...
registration=tk.Button(root,text='For New User Click Here To Register',command=Newregistration)
registration.place(x=190,y=300)
